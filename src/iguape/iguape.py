...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):  # numpydoc ignore=GL08
        ...
        self.props_dict = {
            "Main Axis": {
                ...
                "Cmap_Label": "XRD acquisition order",
            },
            "Peak Fit Axis": {
                "Peak Position Axis": {
                    "X_Label": "XRD acquisition order",
                ...
                "Integrated Area Axis": {
                    "X_Label": "XRD acquisition order",
                    "Y_Label": "Peak Integrated Area (a.u.)",
                    "Cmap_Label": "XRD acquisition order",
                },
            },
            "Contour Axis": {
                "X_Label": "2θ (°)",
                "Y_Label": "XRD acquisition order",
    ...
    def create_graphs_layout(self):
        ...
        self.url_data = {
            self.paineira_logo: "https://lnls.cnpem.br/facilities/paineira-en/",
            self.LNLS_logo: "https://lnls.cnpem.br/en/",
            self.CNPEM_logo: "https://cnpem.br/en/",
            self.iguape_logo: "https://cnpem.github.io/iguape/",
        }
        ...
        self.norm = plt.Normalize(
            vmin=0, vmax=1
        )  # Initial placeholder values for norm #
        self.sm = ScalarMappable(cmap=self.cmap, norm=self.norm)
        self.sm.set_array([])
        self.cax = self.fig_main.colorbar(
            self.sm, ax=self.ax_main
        )  # Creating the colorbar axes #
        self.cax_2 = self.fig_sub.colorbar(
            self.sm, ax=self.ax_area
        )  # Creating the colorbar axes #
        ...
        self.normalization_layout.addWidget(
            NavigationToolbar2QT(self.canvas_norm, self)
        )
    ...
    def _get_mask(self, i: int):
        ...
        if self.selected_interval:
            dois_theta = self.read_data(
                self.plot_data["file_name"][i], Q=self.Q_vector_state
            )[0]
            return (dois_theta >= self.selected_interval[0]) & (
                dois_theta <= self.selected_interval[1]
            )
        return slice(None)
    ...
    def update_colormap(self, color_map_type: str, label: str):
        ...
        self.norm.vmin, self.norm.vmax = (
            min(self.plot_data[color_map_type]),
            max(self.plot_data[color_map_type]),
        )
    ...
    def _update_main_figure(self):
        ...
        try:
            self.plot_data = (
                self.monitor.data_frame[self.temp_mask].reset_index(drop=True)
                if self.temp_mask_signal
                else self.monitor.data_frame
            )
        ...
        if self.plot_with_temp:
            ...
            self.min_temp_doubleSpinBox.setRange(
                min(self.monitor.data_frame["temp"]),
                max(self.monitor.data_frame["temp"]),
            )
            self.max_temp_doubleSpinBox.setRange(
                min(self.monitor.data_frame["temp"]),
                max(self.monitor.data_frame["temp"]),
            )
            ...
            self.update_colormap(
                "file_index", self.props_dict["Main Axis"]["Cmap_Label"]
            )
            self.min_temp_doubleSpinBox.setRange(
                min(self.monitor.data_frame["file_index"]),
                max(self.monitor.data_frame["file_index"]),
            )
            self.max_temp_doubleSpinBox.setRange(
                min(self.monitor.data_frame["file_index"]),
                max(self.monitor.data_frame["file_index"]),
        ...
        for i in range(len(self.plot_data["file_name"])):
            color = self.cmap(
                self.norm(self.plot_data[norm_col][i])
            )  # Selecting the pattern's color based on the colormap
            dois_theta, intensity = self.read_data(
                self.plot_data["file_name"][i], normalize=False, Q=self.Q_vector_state
            )
            if self.plot_with_temp:
                temp = re.search(
                    r"Temperature\s*\(([^)]+)\)",
                    self.props_dict["Main Axis"]["Cmap_Label"],
                ).group(1)
            ...
            self.ax_main.plot(
                dois_theta[mask], intensity[mask] + offset, color=color, label=label
            )
            offset += self.spacing
            del dois_theta, intensity

            self.ax_main.set_xlabel(
                self.props_dict["Main Axis"]["X_Label"], fontsize=15
            )
            self.ax_main.set_ylabel(
                self.props_dict["Main Axis"]["Y_Label"], fontsize=15
            )
        QApplication.restoreOverrideCursor()
        gc.collect()
    ...
    def _plot_fitting_parameters(self):
        ...
        avxspan = self.ax_main.axvspan(
            self.fit_interval[0],
            self.fit_interval[1],
            color="grey",
            alpha=0.5,
            label="Selected Fitting Interval",
        )
    ...
    def _plot_single_peak(self):
        """Hidden method to call :py:meth:`Window._plot_parameter` in order to plot fitting parameters of a single peak."""
        ...
        try:
            self._plot_parameter(
                self.ax_2theta,
                self.monitor.fit_data[x_data_type].values[mask],
                self.monitor.fit_data["dois_theta_0"].values[mask],
                self.props_dict["Peak Fit Axis"]["Peak Position Axis"]["Y_Label"],
                x_label,
            )  # , yerr=self.monitor.fit_data['dois_theta_0_stderr'].values)
            self._plot_parameter(
                self.ax_area,
                self.monitor.fit_data[x_data_type].values[mask],
                self.monitor.fit_data["area"].values[mask],
                self.props_dict["Peak Fit Axis"]["Integrated Area Axis"]["Y_Label"],
                x_label,
            )  # , yerr=self.monitor.fit_data['area_stderr'].values)
            self._plot_parameter(
                self.ax_FWHM,
                self.monitor.fit_data[x_data_type].values[mask],
                self.monitor.fit_data["fwhm"].values[mask],
                self.props_dict["Peak Fit Axis"]["FWHM Axis"]["Y_Label"],
                x_label,
            )  # , yerr=self.monitor.fit_data['fwhm_stderr'].values)

        except IndexError:
            self._plot_parameter(
                self.ax_2theta,
                self.monitor.fit_data[x_data_type].values,
                self.monitor.fit_data["dois_theta_0"].values,
                self.props_dict["Peak Fit Axis"]["Peak Position Axis"]["Y_Label"],
                x_label,
            )  # , yerr=self.monitor.fit_data['dois_theta_0_stderr'].values)
            self._plot_parameter(
                self.ax_area,
                self.monitor.fit_data[x_data_type].values,
                self.monitor.fit_data["area"].values,
                self.props_dict["Peak Fit Axis"]["Integrated Area Axis"]["Y_Label"],
                x_label,
            )  # , yerr=self.monitor.fit_data['area_stderr'].values)
            self._plot_parameter(
                self.ax_FWHM,
                self.monitor.fit_data[x_data_type].values,
                self.monitor.fit_data["fwhm"].values,
                self.props_dict["Peak Fit Axis"]["FWHM Axis"]["Y_Label"],
                x_label,
            )  # , yerr=self.monitor.fit_data['fwhm_stderr'].values)

    def _plot_double_peak(self):
        """Hidden method to call :py:meth:`Window._plot_parameter` in order to plot fitting parameters of a double peak."""
        ...
        try:
            ...
            self._plot_parameter(
                self.ax_2theta,
                self.monitor.fit_data[x_data_type].values[mask],
                self.monitor.fit_data["dois_theta_0_#2"].values[mask],
                self.props_dict["Peak Fit Axis"]["Peak Position Axis"]["Y_Label"],
                x_label,
                label=True,
                color="red",
            ...
            self._plot_parameter(
                self.ax_area,
                self.monitor.fit_data[x_data_type].values[mask],
                self.monitor.fit_data["area_#2"].values[mask],
                self.props_dict["Peak Fit Axis"]["Integrated Area Axis"]["Y_Label"],
                x_label,
                label=True,
                color="green",
            ...
            self._plot_parameter(
                self.ax_FWHM,
                self.monitor.fit_data[x_data_type].values[mask],
                self.monitor.fit_data["fwhm_#2"].values[mask],
                self.props_dict["Peak Fit Axis"]["FWHM Axis"]["Y_Label"],
                x_label,
                label=True,
                color="blue",
            ...
            self._plot_parameter(
                self.ax_2theta,
                self.monitor.fit_data[x_data_type].values,
                self.monitor.fit_data["dois_theta_0_#2"].values,
                self.props_dict["Peak Fit Axis"]["Peak Position Axis"]["Y_Label"],
                x_label,
                label=True,
                color="red",
            ...
            self._plot_parameter(
                self.ax_area,
                self.monitor.fit_data[x_data_type].values,
                self.monitor.fit_data["area_#2"].values,
                self.props_dict["Peak Fit Axis"]["Integrated Area Axis"]["Y_Label"],
                x_label,
                label=True,
                color="green",
            ...
            self._plot_parameter(
                self.ax_FWHM,
                self.monitor.fit_data[x_data_type].values,
                self.monitor.fit_data["fwhm_#2"].values,
                self.props_dict["Peak Fit Axis"]["FWHM Axis"]["Y_Label"],
                x_label,
                label=True,
                color="blue",
    ...
    def _plot_parameter(
        self, ax, x, y, ylabel, xlabel, label=None, color=None, marker="o", yerr=None
    ):
        """
    ...
    def select_folder(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(
            self, "Select the data folder to monitor", "", options=QFileDialog.Options()
        )  # Selection of monitoring folder
        if folder_path == "":
            return
        if self.folder_selected:
            self.monitor.data_frame = pd.DataFrame(
                columns=["file_name", "temp", "max", "file_index"]
            )
            self.monitor.fit_data = pd.DataFrame(
                columns=[
                    "dois_theta_0",
                    "fwhm",
                    "area",
        ...
        if folder_path:
            if not (os.path.exists(os.path.join(folder_path, "iguape_filelist.txt"))):
                QMessageBox.warning(
                    self,
                    "",
                    "This folder does not contain the iguape_filelist.txt file! Please select a valid folder!",
                )
                return
    ...
    def onselect(self, xmin, xmax):
        ...
        It sets the selected interval for visualization based on the min/max value received for 2theta.
        ...
        if not self.folder_selected:
            ...
            try:
                if len(self.plot_data["file_name"]) == 0:
                    ...
                    self.ax_FWHM.clear()
                    self.fit_interval = None
                    self.monitor.fit_data = self.monitor.fit_data.iloc[
                        0:0
                    ]  # Reset fitting data
    ...
    def on_mouse_move(self, event):
        ...
        if label is not None and x is not None and y is not None:
            s = rf"Label: {label} | {self.props_dict['Main Axis']['X_Label']}={x:.2f}, {self.props_dict['Main Axis']['Y_Label']}={y:.2e}"
            ...
            if df is not None:
                ...
                if file_path:
                    with open(file_path, "w+") as f:
                        f.write(
                            "# For more information on each peak fit, please, refer to IGUAPE's terminal window, where you will find a complete report on each fit.\n"
                        )
                        f.write(
                            "# If uncertainties were not generated for one or multiple fits, it probably beacause one or more parameters reached a boundary value. For more information refer to lmfit's documentation: https://lmfit.github.io/lmfit-py/faq.html\n"
                        )
            ...
            return pd.DataFrame(
                {
                    ...
                    self.props_dict["Peak Fit Axis"]["Peak Position Axis"][
                        "Y_Label"
                    ]: self.monitor.fit_data["dois_theta_0"],
                    f"{self.props_dict['Peak Fit Axis']['Peak Position Axis']['Y_Label']} std": self.monitor.fit_data[
                        "dois_theta_0_std"
                    ],
                    self.props_dict["Peak Fit Axis"]["Integrated Area Axis"][
                        "Y_Label"
                    ]: self.monitor.fit_data["area"],
                    f"{self.props_dict['Peak Fit Axis']['Integrated Area Axis']['Y_Label']} std": self.monitor.fit_data[
                        "area_std"
                    ],
                    self.props_dict["Peak Fit Axis"]["FWHM Axis"][
                        "Y_Label"
                    ]: self.monitor.fit_data["fwhm"],
                    f"{self.props_dict['Peak Fit Axis']['FWHM Axis']['Y_Label']} std": self.monitor.fit_data[
                        "fwhm_std"
                    ],
    ...
    def _create_double_peak_dataframe(self):
        ...
        if self.plot_with_temp:
            ...
            return pd.DataFrame(
                {
                    temp_label: self.monitor.fit_data["temp"],
                    f"{self.props_dict['Peak Fit Axis']['Peak Position Axis']['Y_Label']} #1": self.monitor.fit_data[
                        "dois_theta_0"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['Peak Position Axis']['Y_Label']} #1 std": self.monitor.fit_data[
                        "dois_theta_0_std"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['Integrated Area Axis']['Y_Label']} #1": self.monitor.fit_data[
                        "area"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['Integrated Area Axis']['Y_Label']} #1 std": self.monitor.fit_data[
                        "area_std"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['FWHM Axis']['Y_Label']} #1": self.monitor.fit_data[
                        "fwhm"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['FWHM Axis']['Y_Label']} #1 std": self.monitor.fit_data[
                        "fwhm_std"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['Peak Position Axis']['Y_Label']} #2": self.monitor.fit_data[
                        "dois_theta_0_#2"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['Peak Position Axis']['Y_Label']} #2 std": self.monitor.fit_data[
                        "dois_theta_0_#2_std"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['Integrated Area Axis']['Y_Label']} #2": self.monitor.fit_data[
                        "area_#2"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['Integrated Area Axis']['Y_Label']} #2 std": self.monitor.fit_data[
                        "area_#2_std"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['FWHM Axis']['Y_Label']} #2": self.monitor.fit_data[
                        "fwhm_#2"
                    ],
                    f"{self.props_dict['Peak Fit Axis']['FWHM Axis']['Y_Label']} #2 std": self.monitor.fit_data[
                        "fwhm_#2_std"
                    ],
                    "R-squared (R²)": self.monitor.fit_data["R-squared"],
                }
    ...
    def validate_temp(self, min_value, max_value):
        ...
        """
        min_temp = min(
            self.monitor.data_frame["temp"], key=lambda x: abs(x - min_value)
        )
        max_temp = min(
            self.monitor.data_frame["temp"], key=lambda x: abs(x - max_value)
        )
        return min_temp, max_temp

    def apply_temp_mask(self, mask):
        """Method for applying a temperature mask in the XRD measures.
        Recieves an array of booleans. The array has a length equal to the number of XRD data read by the :py:class:`Iguape.Monitor.FolderMonitor`.
        Args:
            mask (np.array): numpy array with booleans as entries
        """
    ...
    def measure_order_index(self, checked):
        """Method connected to the measure order QCheckBox. This method will change the colormaps that identify the XRD data.
        All of the Figures are updated accordingly.
    ...
    def temp_index(self, checked):
        ...
        try:
            if checked:
                if self.monitor.data_frame["temp"][0] is not None:
    ...
    def read_data(self, path, normalize=False, Q=False):
        """Reads the XRD data using pandas *read_csv* function.
        Comments are identified by "#" and the header is identified by the first line.
        ...
        Returns:
            (theta, intensity): 2theta and Intensity numpy arrays read from the XRD data.
        """
    ...
    def normalize(self):
        ...
        try:
            ...
            for i in range(len(self.plot_data["file_name"])):
                norm_col = (
                    "temp" if self.plot_with_temp else "file_index"
                )  # Flag for chosing the XRD pattern index
                color = self.cmap(
                    self.norm(self.plot_data[norm_col][i])
                )  # Selecting the pattern's color based on the colormap
                dois_theta, intensity = self.read_data(
                    self.plot_data["file_name"][i],
                    normalize=True,
                    Q=self.Q_vector_state,
                )
                self.ax_norm.plot(
                    dois_theta[mask],
                    intensity[mask] + offset,
                    color=color,
                    label=f"XRD pattern #{self.plot_data['file_index'][i]} - Temperature {self.plot_data['temp'][i]} K"
                    if self.plot_with_temp
                    else f"XRD pattern #{self.plot_data['file_index'][i]}",
                )
                offset += spacing
                del dois_theta, intensity
            ...
            if self.plot_with_temp:
                self.update_colormap(
                    "temp", self.props_dict["Normalization Axis"]["Cmap_Label"]
                )
            else:
                self.update_colormap("file_index", "XRD acquisition time")

            self.cax_4.set_label(
                self.props_dict["Normalization Axis"]["Cmap_Label"], fontsize=15
            )
            self.ax_norm.set_xlabel(
                self.props_dict["Normalization Axis"]["X_Label"], fontsize=15
            )
            self.ax_norm.set_ylabel(
                self.props_dict["Normalization Axis"]["Y_Label"], fontsize=15
            )
    ...
    def contour(self):
        ...
        try:
            self.ax_contour.clear()
            theta, intensity = self.read_data(
                self.plot_data["file_name"][0], Q=self.Q_vector_state
            )
            if self.plot_with_temp:
            ...
            self.ax_contour.set_ylabel(
                self.props_dict["Contour Axis"]["Y_Label"], fontsize=15
            )
            mask = self._get_mask(0)
            ...
            colormesh = self.ax_contour.pcolormesh(
                X,
                Y,
                z,
                cmap=self.color_pallete_comboBox.currentText(),
                norm=norm,
                shading="auto",
            )
            self.cax_3.update_normal(colormesh)
            self.cax_3.set_label(
                self.props_dict["Contour Axis"]["Cmap_Label"], fontsize=15
            )
            self.ax_contour.set_xlabel(
                self.props_dict["Contour Axis"]["X_Label"], fontsize=15
            )
    ...
    def apply_filter(self):
        """Initializes the :py:class:`FilterWindow` class."""
        if not self.monitor:
            return
        try:
            self.filter_window = FilterWindow(
                self.monitor.data_frame.iloc[:, 0:3], self.monitor.kelvin_sginal
            )
            self.filter_window.mask.connect(self.apply_temp_mask)
            self.filter_window.show()
        except AttributeError or Exception:
            QMessageBox.warning(
                self, "No folder was selected", "Select a folder to monitor!"
            )

    def on_toggle_Q_vector_action(self):
        """
        This is triggred when the Q vector visualization option is selected by the user.
        This renders a QDialog for user input of the wavelength and than updates the graphs to plot as function of Q.
        """
        dialog = QInputDialog()
        ...
        if (not ok) or (dialog.doubleValue() == 0):
            ...
            QMessageBox.warning(
                self,
                "Invlid Wavelenght",
                "Please insert a valid wavelength for Q vector computation",
            )
            return
        else:
    ...
    def on_toggle_2theta_action(self):
        """
        This is triggred when the 2theta visualization option is selected by the user.
        This is the default visualization mode. The method updates the graphs to plot as function of 2theta.
        """
        self.actionQ_Vector.setChecked(False)
    ...
    def on_change_vline_checkbox(self):
        ...
        if self.cursor is None:
            self.cursor = Cursor(
                self.ax_main, useblit=True, color="red", linewidth=1, horizOn=False
            )
        else:
            self.cursor = None
...
class GarbageCollector(QThread):
    """Garbage Collector class. Inherits a QThread and calls, every 3 seconds, the collect method of python's garbage collector.
    The matpltolib Figures are updated using **for loops**. This consumes a lot of RAM memory. This method frees up some memory space.
...
class Worker(QThread):
    """QThread that performs peak fitting in the selected XRD data.
    Args:
        QThread (QThread): QThread class from PyQt5
    """
    ...
    error = Signal(str)  # Changed to emit multiple arrays

    def __init__(self, interval):
        r"""Constructor for the peak fitting thread.

        Args:
            interval (list): List in the form [:math:`2\theta_{i}`, :math:`2\theta_{f}`] where :math:`2\theta_{i}` and :math:`2\theta_{f}` are, respectively,
            the initial and final values of the 2theta interval selected to perfor peak fitting.
        """
        super().__init__()
    ...
    def run(self):
        r"""Performs peak fitting in the selected XRD data in the selected 2theta interval [:math:`2\theta_{i}`, :math:`2\theta_{f}`]"""

        try:
            start = time.time()
            pars = None
            for i in range(len(win.plot_data["file_name"])):
                theta, intensity = win.read_data(
                    win.plot_data["file_name"][i], Q=win.Q_vector_state
                )
                ...
                if win.fit_interval_window.fit_model == "PseudoVoigt":
                    ...
                    try:
                        dois_theta_std, fwhm_std, area_std = (
                            fit[7]["center"].stderr * 1,
                            fit[7]["fwhm"].stderr * 1,
                    ...
                    new_fit_data = pd.DataFrame(
                        {
                            "dois_theta_0": [fit[0]],
                            "dois_theta_0_std": [dois_theta_std],
                            "fwhm": [fit[1]],
                            "fwhm_std": [fwhm_std],
                            "area": [fit[2]],
                            "area_std": [area_std],
                            "temp": [win.plot_data["temp"][i]],
                            "file_index": [win.plot_data["file_index"][i]],
                            "R-squared": [fit[3]],
                        }
                    )
                    win.monitor.fit_data = pd.concat(
                        [win.monitor.fit_data, new_fit_data], ignore_index=True
                    )
                    pars = fit[7]
                    progress_value = int(
                        (i + 1) / len(win.plot_data["file_name"]) * 100
                    )
                    self.progress.emit(
                        progress_value
                    ...
                    fit = peak_fit_split_gaussian(  # noqa: F821
                        ...
                        self.fit_interval,
                        id=id,
                        height=win.fit_interval_window.height,
                        distance=win.fit_interval_window.distance,
                        prominence=win.fit_interval_window.prominence,
                        pars=pars,
                    )
                    try:
                        (
                            ...
                            fit[7]["cen1"].stderr * 1,
                            fit[7]["sigma1"].stderr * 2,
                            fit[7]["amp1"].stderr * 1,
                            fit[7]["cen2"].stderr * 1,
                            fit[7]["sigma2"].stderr * 2,
                            fit[7]["amp2"].stderr * 1,
                        )
                    except Exception as e:
                        print(f"Exception {e}")
                    ...
                    new_fit_data = pd.DataFrame(
                        {
                            "dois_theta_0": [fit[0][0]],
                            "dois_theta_0_std": [dois_theta_std],
                            "dois_theta_0_#2": [fit[0][1]],
                            "dois_theta_0_#2_std": [dois_theta_2_std],
                            "fwhm": [fit[1][0]],
                            "fwhm_std": [fwhm_std],
                            "fwhm_#2": [fit[1][1]],
                            "fwhm_#2_std": [fwhm_2_std],
                            "area": [fit[2][0]],
                            "area_std": [area_std],
                            "area_#2": [fit[2][1]],
                            "area_#2_std": [area_2_std],
                            "temp": [win.plot_data["temp"][i]],
                            "file_index": [win.plot_data["file_index"][i]],
                            "R-squared": [fit[3]],
                        }
            ...
            self.error.emit(
                f"Error during peak fitting: {str(e)}. Please select a new Fit Interval"
            )
...
class FilterWindow(QDialog, Ui_Filter_Dialog):
    ...
    def __init__(self, data=None, kelvin_signal=None, parent=None):
        """Constructor
        This method will initialize the QDialog and render a QListView() in the QDialog Window.
    ...
    def apply(self):
        ...
        if True in list(self.model._data.iloc[:, -1]):
            ...
            QMessageBox.warning(
                ...
                "Please, select at least one XRD diffractogram!",
            )
            return
...
class ExportWindow(QDialog, Ui_Export_Figure):
    ...
    def __init__(self, figure: Figure, parent=None):
        ...
        self.label_style_comboBox.currentTextChanged.connect(
            self.on_change_label_style_comboBox
        )
        self.ticks_size_spinBox.valueChanged.connect(self.on_change_tick_size_spinBox)
        self.ticks_style_comboBox.currentTextChanged.connect(
    ...
    def redraw_fig(self):
        ...
        try:
            ...
            print("{e}")
            return
        tickfont = matplotlib.font_manager.FontProperties(
            family=self.tick_font["family"],
        ...
        self.cmap_ax.set_ylabel(
            self.cmap_label_lineEdit.text(), fontdict=self.label_font
        )
    ...
    def save_fig(self):
        ...
        if path == "":
            QMessageBox.warning(
                self, "Saving Error", "Please, select a valid path for your Figure"
            )
            return
        print(path)
        # self.canvas.resize(int(width*self.edit_fig.dpi), int(height*self.edit_fig.dpi))
        try:  # try-except to handle save when height and width are None
            ...
            pass
        self.canvas.draw()
        self.edit_fig.savefig(
            path + f".{self.format_comboBox.currentText()}",
...
class FitWindow(QDialog, Ui_pk_window):
    ...
    def setup_layout(self):
        ...
        theta, intensity = win.read_data(
            win.plot_data["file_name"][0], Q=win.Q_vector_state
        )
        if win.plot_with_temp:
            ...
            self.ax.plot(
                ...
                "o",
                markersize=3,
                label="XRD pattern " + str(win.plot_data["temp"][0]) + "°C",
            )
            ...
            self.ax.plot(
                ...
                "o",
                markersize=3,
                label="XRD pattern #" + str(win.plot_data["file_index"][0]),
            )
        ...
        self.prominence_spinBox.valueChanged[int].connect(
            self.onChanged_prominence_spinbox
        )
    ...
    def onChanged_xrd_combo_box(self, text):
        ...
        if len(self.ax.lines) == 2:
            QMessageBox.warning(
                self,
                "",
                "Warning! It is possible to display only two XRD patterns in this window! Please press the Clear Plot button and select up to 2 XRD patterns to be displayed.",
            )
            pass
        else:
            ...
            if win.plot_with_temp:
                self.ax.plot(
                    theta, intensity, "o", markersize=3, label=("XRD pattern " + text)
                )
    ...
    def preview(self):
        ...
        if self.fit_model == "PseudoVoigt":
            for i in range(len(self.indexes)):
                theta, intensity = win.read_data(
                    win.plot_data["file_name"][self.indexes[i]], Q=win.Q_vector_state
                )
                id = [
                ...
                if win.plot_with_temp:
                    self.ax.plot(
                        ...
                        best_fit,
                        "--",
                        label=f"Best Fit - {win.plot_data['temp'][self.indexes[i]]} °C",
                    )
                    self.ax.plot(
                        data[6],
                        data[5]["bkg_"],
                        "-",
                        label=f"Background - {win.plot_data['temp'][self.indexes[i]]} °C",
                    )
                    ...
                    self.ax.plot(
                        ...
                        data[4].best_fit,
                        "--",
                        label=f"Best Fit - #{win.plot_data['file_index'][self.indexes[i]]}",
                    )
                    self.ax.plot(
                        data[6],
                        data[5]["bkg_"],
                        "-",
                        label=f"Background - #{win.plot_data['file_index'][self.indexes[i]]}",
                    )
                    # self.ax.fill_between(data[6],best_fit-dely, best_fit+dely, color = "darkslategrey", alpha=0.9, label=r"3-$\sigma$ uncertainty band")
                self.ax.legend(fontsize="small")
                self.canvas.draw()
            ...
            for i in range(len(self.indexes)):
                try:
                    theta, intensity = win.read_data(
                        win.plot_data["file_name"][self.indexes[i]],
                        Q=win.Q_vector_state,
                    )
                    id = [
                        win.plot_data["file_index"][self.indexes[i]],
                        win.plot_data["temp"][self.indexes[i]],
                    ...
                    QMessageBox.warning(
                        self,
                        "",
                        "The value given for distance and/or height for peak search are out of bounds, i.e., it was not possible to find two peaks mtaching the given parameters! Please, try again with different values for distance and height!",
                    )
                except TypeError:
                    QMessageBox.warning(
                        self,
                        "",
                        "One or more peak parameters have reached a boundary value! Please check IGUAPE terimnal window for a fit report!",
                    )
    ...
    def peak_fitting_finished(self, time):
        ...
        QMessageBox.information(
            self,
            "Peak Fitting",
            f"Peak fitting completed successfully! For more information on each fit, check the terminal for the fit report! Elapsed time: {int(time)}s",
        )
        win.update_graphs()
